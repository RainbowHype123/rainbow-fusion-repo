...
#import "CExtension.h"
#import "CRunBehind_The_Runtime.hpp"

@implementation CRunBehind_The_Runtime : CRunExtension

-(id)init
{
	if (++initedBehind_The_RuntimeCPP == 1)
		Behind_The_Runtime_init();
	return [super init];
}
-(void)dealloc
{
	...
	if (--initedBehind_The_RuntimeCPP == 0)
		Behind_The_Runtime_dealloc();
	[super dealloc];
}
-(int)getNumberOfConditions
{
	...
	return Behind_The_Runtime_getNumberOfConditions();
}
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo *)cob andVersion:(int)version
{
	if (initedBehind_The_RuntimeCPP == 0)
		NSLog(@"Init for Behind The Runtime was never run!");
	NSLog(@"CreateRunObject for Behind_The_Runtime");
	...
	cptr = Behind_The_Runtime_createRunObject(edPtr, cob, version, self);
	if (!cptr)
		NSLog(@"Failed to createRunObject for Behind The Runtime!");
	free(edPtr);
	return cptr != NULL;
}
-(int)handleRunObject
{
	return Behind_The_Runtime_handleRunObject(cptr);
}
-(void)destroyRunObject:(BOOL)bFast
{
	Behind_The_Runtime_destroyRunObject(cptr, bFast);
	cptr = NULL;
	[expRet dealloc];
}
-(BOOL)condition:(int)paramNum withCndExtension:(CCndExtension*)cnd
{
	...
	// by checking num and the return of Behind_The_Runtime_conditionJump, which is either a 32-bit integer,
	...
	return Behind_The_Runtime_conditionJump(cptr, paramNum, cnd) != 0;
}
-(void)action:(int)paramNum withActExtension:(CActExtension*)act
{
	Behind_The_Runtime_actionJump(cptr, paramNum, act);
}
-(CValue*)expression:(int)paramNum
{
	Behind_The_Runtime_expressionJump(cptr, paramNum);
	return expRet;
}
...
extern "C" {
	void DarkEdif_Behind_The_Runtime_generateEvent(void * ext, int code, int param)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		[extRun->ho generateEvent:code withParam:param];
	}
	void DarkEdif_Behind_The_Runtime_reHandle(void * ext)
	{
		CRunExtension * extRun = (CRunBehind_The_Runtime *)ext;
		[extRun->ho reHandle];
	}

	int DarkEdif_Behind_The_Runtime_actGetParamExpression(void * ext, void * act, int paramNum, int paramType)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		CActExtension * actExt = (CActExtension *)act;

		// ParamObject is 1, we return the oiList, which can be qualifier
		if (paramType == 1)
			return actExt->pParams[paramNum]->evp.evpL.evpL0;

		// ParamTime is 2, but can sometimes be an expression, which getParamTime handles
		if (paramType == 2)
			return [actExt getParamTime:extRun->rh withNum:paramNum];
		...
		if (paramType == 29)
		{
			...
			{
				printf("Behind_The_Runtime: DarkEdif_actGetParamExpression passed a direction %d which cannot translate into NewDirection bitmask. Using no direction.", result);
				return 0;
			}
			return 1 << result;
		}
		...
		if (paramType != 22)
			printf("Behind_The_Runtime: DarkEdif_actGetParamExpression: Unknown param type %d requested. Defaulting to Expression.", paramType);
		return [actExt getParamExpression:extRun->rh withNum:paramNum];
	}
	const char * DarkEdif_Behind_The_Runtime_actGetParamExpString(void * ext, void * act, int paramNum)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		CActExtension * actExt = (CActExtension *)act;
	...
	double DarkEdif_Behind_The_Runtime_actGetParamExpDouble(void * ext, void * act, int paramNum)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		CActExtension * actExt = (CActExtension *)act;
	...
	int DarkEdif_Behind_The_Runtime_cndGetParamExpression(void * ext, void * cnd, int paramNum, int paramType)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		...
		// ParamObject is 1, we return the oiList, which can be qualifier
		if (paramType == 1)
			return [cndExt getParamOi:extRun->rh withNum:paramNum];

		// ParamTime is 2, but can sometimes be an expression, which getParamTime handles
		if (paramType == 2)
			return [cndExt getParamTime:extRun->rh withNum:paramNum];
		...
		if (paramType == 29)
		{
			...
			if (result < 0 || result > 31)
			{
				printf("Behind_The_Runtime: DarkEdif_cndGetParamExpression passed a direction %d which cannot translate into NewDirection bitmask. Using no direction.", result);
				return 0;
			}
			return 1 << result;
		}
		...
		// ParamExpression is 22; at the moment, no other types are expected
		if (paramType != 22)
			printf("Behind_The_Runtime: DarkEdif_cndGetParamExpression: Unknown param type %d requested. Defaulting to Expression.", paramType);
		return [cndExt getParamExpression:extRun->rh withNum:paramNum];
	}
	const char * DarkEdif_Behind_The_Runtime_cndGetParamExpString(void * ext, void * cnd, int paramNum)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		CCndExtension * cndExt = (CCndExtension *)cnd;
	...
	double DarkEdif_Behind_The_Runtime_cndGetParamExpDouble(void * ext, void * cnd, int paramNum)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		CCndExtension * cndExt = (CCndExtension *)cnd;
	...
	int DarkEdif_Behind_The_Runtime_expGetParamInt(void * ext)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		return [[extRun->ho getExpParam] getInt];
	}
	const char * DarkEdif_Behind_The_Runtime_expGetParamString(void * ext)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		return [[[extRun->ho getExpParam] getString] UTF8String];
	}
	float DarkEdif_Behind_The_Runtime_expGetParamFloat(void * ext)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		return (float)[[extRun->ho getExpParam] getDouble];
	}

	void DarkEdif_Behind_The_Runtime_expSetReturnInt(void * ext, int i)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		[extRun->expRet forceInt:i];
	}
	void DarkEdif_Behind_The_Runtime_expSetReturnString(void * ext, const char * str)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		[extRun->expRet forceString:[NSString stringWithUTF8String:str]];
	}
	void DarkEdif_Behind_The_Runtime_expSetReturnFloat(void * ext, float f)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		[extRun->expRet forceDouble:(double)f];
	}

	void DarkEdif_Behind_The_Runtime_freeString(void * ext, const char * cstr)
	{
		[(NSString*)cstr release];
	}

	int DarkEdif_Behind_The_Runtime_getCurrentFusionEventNum(void * ext)
	{
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
	...
	char * DarkEdif_Behind_The_Runtime_makePathUnembeddedIfNeeded(void * ext, const char * fileName)
	{
		...
		CRunBehind_The_Runtime * extRun = (CRunBehind_The_Runtime *)ext;
		const NSString * const filenameNS = [NSString stringWithUTF8String:fileName];
	...
	void * DarkEdif_Behind_The_Runtime_getCPtr(void * ext)
	{
		CRunBehind_The_Runtime * const extRun = (CRunBehind_The_Runtime *)((CExtension *)ext)->ext;

		// While we cast this to CRunBehind_The_Runtime, it may be any sort of CRun, including one that doesn't have cptr.
		...
		{
			printf("Behind_The_Runtime: DarkEdif_getCPtr passed an invalid, non-DarkEdif ext.\n");
			return NULL;
		}
		return extRun->cptr;
	}
