/* File generated by DarkEdifPostBuildTool v1.0.1.1, part of DarkEdif SDK. 
   ...
   Copyright of the Behind The Runtime extension and all rights reserved by the creator(s) of Behind The Runtime.
   
   A native extension needs a Java file to generate a Java class. The class is empty,
   and a native is assumed by presence of CRunBehind_The_Runtime in the assets folder.
...
public class CRunBehind_The_Runtime extends CRunExtension
{
	...
	public CRunBehind_The_Runtime()
	{
		if (!inited)
		{
			String libName = "Behind_The_Runtime";

			try
			{
				android.util.Log.v("Behind_The_Runtime", "Loading Behind The Runtime extension from \"app/main/jniLibs/" + MMFRuntime.inst.ABI + "/lib" + libName + ".so\"...");
				System.loadLibrary(libName);

				android.util.Log.v("Behind_The_Runtime", "Loaded Behind The Runtime extension OK!");

				inited = true;
			}
			catch (UnsatisfiedLinkError e)
			{
				android.util.Log.e("Behind_The_Runtime", "Couldn't load extension Behind The Runtime, library lib" + libName + ".so couldn't be loaded: " + e.toString());
			}
		}
	}
	...
	public boolean createRunObject(CBinaryFile file, CCreateObjectInfo cob, int version)
	{
		...
		if (cptr == 0)
			android.util.Log.e("Behind_The_Runtime", "Behind The Runtime's createRunObject returned NULL!");
	...
	public int darkedif_jni_getActionOrConditionIntParam(CEvent evt, int paramIndex, int paramType)
	{
		if (!(evt instanceof CCndExtension) && !(evt instanceof CActExtension))
			android.util.Log.e("Behind_The_Runtime", "darkedif_jni_getActionOrConditionIntParam: The CEvent is not CActExtension or CCndExtension. Type is \"" + evt.getClass().getSimpleName() + "\".");
		if (paramIndex == -1)
			android.util.Log.e("Behind_The_Runtime", "darkedif_jni_getActionOrConditionIntParam: paramIndex -1.");
		...
		if (paramType == 29)
		{
			...
			{
				android.util.Log.e("Behind_The_Runtime", "darkedif_jni_getActionOrConditionIntParam: Passed a direction " + result + " which cannot translate into NewDirection bitmask. Using no direction.");
				return 0;
			}
			return 1 << result;
		}
		...
		if (paramType != 22)
			android.util.Log.w("Behind_The_Runtime", "darkedif_jni_getActionOrConditionIntParam: Unknown param type " + paramType + " requested. Defaulting to Expression.");
		return this.rh.get_EventExpressionInt((CParamExpression)evt.evtParams[paramIndex]);
	}
}
