...
#include <shellapi.h> // For the shell execute actions
...
#endif
{
	// Properties (moved up to top)
	//bool killJdkOnAppStart = edPtr->Props.IsPropChecked("Terminate JDK on object creation"sv);
	...
	*/
	...
	LinkAction(0, TerminateJdk);
	LinkCondition(0, OnJdkTerminated);
	LinkExpression(0, AutoKillJdkProp);
	/*
	LinkAction(0, TerminateJdk);
	LinkAction(1, TerminateJdkAlt);

	LinkCondition(0, IsJdkActive);
	LinkCondition(1, OnJdkTerminated);

	LinkExpression(0, JdkActiveFlag);
	LinkExpression(1, AutoKillJdkProp);
	*/
	...
	*/
	...
	killJdkOnAppStart = edPtr->Props.IsPropChecked("Terminate JDK on object creation"sv);
