/* File generated by DarkEdifPostBuildTool v1.0.1.1, part of DarkEdif SDK. 
   ...
   Copyright of the UUID Generator extension and all rights reserved by the creator(s) of UUID Generator.
   
   A native extension needs a Java file to generate a Java class. The class is empty,
   and a native is assumed by presence of CRunUUID_Generator in the assets folder.
...
public class CRunUUID_Generator extends CRunExtension
{
	private static boolean inited = false;
	private long cptr; // For 32-bit and 64-bit
	public CRunUUID_Generator()
	{
		if (!inited)
		{
			String libName = "UUID_Generator";

			try
			{
				android.util.Log.v("UUID_Generator", "Loading UUID Generator extension from \"app/main/jniLibs/" + MMFRuntime.inst.ABI + "/lib" + libName + ".so\"...");
				System.loadLibrary(libName);

				android.util.Log.v("UUID_Generator", "Loaded UUID Generator extension OK!");

				inited = true;
			}
			catch (UnsatisfiedLinkError e)
			{
				android.util.Log.e("UUID_Generator", "Couldn't load extension UUID Generator, library lib" + libName + ".so couldn't be loaded: " + e.toString());
			}
		}
	}
	...
	public boolean createRunObject(CBinaryFile file, CCreateObjectInfo cob, int version)
	{
		...
		if (cptr == 0)
			android.util.Log.e("UUID_Generator", "UUID Generator's createRunObject returned NULL!");
	...
	public int darkedif_jni_getActionOrConditionIntParam(CEvent evt, int paramIndex, int paramType)
	{
		if (!(evt instanceof CCndExtension) && !(evt instanceof CActExtension))
			android.util.Log.e("UUID_Generator", "darkedif_jni_getActionOrConditionIntParam: The CEvent is not CActExtension or CCndExtension. Type is \"" + evt.getClass().getSimpleName() + "\".");
		if (paramIndex == -1)
			android.util.Log.e("UUID_Generator", "darkedif_jni_getActionOrConditionIntParam: paramIndex -1.");
		...
		if (paramType == 29)
		{
			...
			{
				android.util.Log.e("UUID_Generator", "darkedif_jni_getActionOrConditionIntParam: Passed a direction " + result + " which cannot translate into NewDirection bitmask. Using no direction.");
				return 0;
			}
			return 1 << result;
		}
		...
		if (paramType != 22)
			android.util.Log.w("UUID_Generator", "darkedif_jni_getActionOrConditionIntParam: Unknown param type " + paramType + " requested. Defaulting to Expression.");
		return this.rh.get_EventExpressionInt((CParamExpression)evt.evtParams[paramIndex]);
	}
}
