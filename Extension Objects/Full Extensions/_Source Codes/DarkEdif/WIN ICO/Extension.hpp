...
public:
	...
	*/
	...
	HICON GetOriginalIcon(HWND hwnd, int iconType)
	{
		HICON hIcon = (HICON)SendMessage(hwnd, WM_GETICON, iconType, 0);
		...
		return (HICON)GetClassLongPtr(
			hwnd,
			iconType == ICON_SMALL ? GCLP_HICONSM : GCLP_HICON
		);
	}
	/*
	TCHAR * lastSmallWindowICO;
	TCHAR * lastBigWindowICO;
	
	TCHAR * lastErr;
	int lastErrCode;
	*/
	HICON originalSmallIcon;
	HICON originalBigIcon;

	HICON currentSmallIcon;
	HICON currentBigIcon;
	...
	*/
	...
	void SetIcons(TCHAR * filePath);
	void SetTaskbarIcon(TCHAR * filePath);
	void ResetIcons();
	void SetAutoResetProp(int propInput);

	// Conditions

	bool OnIconsChanged();
	bool OnTaskbarIconChanged();
	bool OnIconsReset();

	// Expressions

	const TCHAR * GetMmfHwnd();
	const TCHAR * GetHeaderIconHwnd();
	const TCHAR * GetTaskbarIconHwnd();
	const TCHAR * GetOGHeaderIconHwnd();
	const TCHAR * GetOGTaskbarIconHwnd();
	int GetAutoResetProp();
