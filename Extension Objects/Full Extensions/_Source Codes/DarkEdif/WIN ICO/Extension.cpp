...
#endif
{
	...
	*/
	...
	LinkAction(0, SetIcons);
	LinkAction(1, SetTaskbarIcon);
	LinkAction(2, ResetIcons);
	LinkAction(3, SetAutoResetProp);

	LinkCondition(0, OnIconsChanged);
	LinkCondition(1, OnTaskbarIconChanged);
	LinkCondition(2, OnIconsReset);

	LinkExpression(0, GetMmfHwnd);
	LinkExpression(1, GetHeaderIconHwnd);
	LinkExpression(2, GetTaskbarIconHwnd);
	LinkExpression(3, GetOGHeaderIconHwnd);
	LinkExpression(4, GetOGTaskbarIconHwnd);
	LinkExpression(5, GetAutoResetProp);
	...
	*/
	...
	/*
	lastSmallWindowICO = _T("");
	lastBigWindowICO = _T("");
	lastErr = _T("N/A");
	lastErrCode = 0;
	*/
	originalSmallIcon = GetOriginalIcon(rhPtr->rhHMainWin, ICON_SMALL);
	originalBigIcon = GetOriginalIcon(rhPtr->rhHMainWin, ICON_BIG);

	currentSmallIcon = originalSmallIcon;
	currentBigIcon = originalBigIcon;
	...
	autoResetIcons = edPtr->Props.IsPropChecked("Auto-Reset icons on object destruction"sv);
...
Extension::~Extension()
{
	...
	//ResetIcons(); // Reset icons to defaults
	
	//if (currentSmallIcon)
    //    DestroyIcon(currentSmallIcon);

	//currentSmallIcon = nullptr;

    //if (currentBigIcon)
    //    DestroyIcon(currentBigIcon);

	//currentBigIcon = nullptr;
}
