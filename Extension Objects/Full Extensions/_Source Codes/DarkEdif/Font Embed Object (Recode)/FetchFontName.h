// Credit to Defisym for the GetFontName.h file, which I based this file off of. -RH
...
inline FontNames GetFontFamilyNames(LPCWSTR lpszFilePath) 
{
    ...
    if (fp != nullptr) 
    {
		...
		// Check is this is a TrueType font and the version is 1.0 -Defisym
		if (ttOffsetTable.uMajorVersion != 1 || ttOffsetTable.uMinorVersion != 0) 
        {
			fclose(fp); // Modification from Defisym's implementation to prevent a possible memory leak
            return fontNames;
		}
				...
				if (ttRecord.uNameID == 1) 
                {
                    ...
                    {						
						...
						if (pwstr != NULL) 
                        {
                            ...
                            // (Unused code from Defisym's file)
