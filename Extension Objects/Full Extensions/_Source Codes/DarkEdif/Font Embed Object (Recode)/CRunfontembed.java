/* File generated by DarkEdifPostBuildTool v1.0.1.1, part of DarkEdif SDK. 
   ...
   Copyright of the Font Embed Object extension and all rights reserved by the creator(s) of Font Embed Object.
   
   A native extension needs a Java file to generate a Java class. The class is empty,
   and a native is assumed by presence of CRunfontembed in the assets folder.
...
public class CRunfontembed extends CRunExtension
{
	private static boolean inited = false;
	private long cptr; // For 32-bit and 64-bit
	public CRunfontembed()
	{
		if (!inited)
		{
			String libName = "fontembed";

			try
			{
				android.util.Log.v("fontembed", "Loading Font Embed Object extension from \"app/main/jniLibs/" + MMFRuntime.inst.ABI + "/lib" + libName + ".so\"...");
				System.loadLibrary(libName);

				android.util.Log.v("fontembed", "Loaded Font Embed Object extension OK!");

				inited = true;
			}
			catch (UnsatisfiedLinkError e)
			{
				android.util.Log.e("fontembed", "Couldn't load extension Font Embed Object, library lib" + libName + ".so couldn't be loaded: " + e.toString());
			}
		}
	}
	...
	public boolean createRunObject(CBinaryFile file, CCreateObjectInfo cob, int version)
	{
		...
		if (cptr == 0)
			android.util.Log.e("fontembed", "Font Embed Object's createRunObject returned NULL!");
	...
	public int darkedif_jni_getActionOrConditionIntParam(CEvent evt, int paramIndex, int paramType)
	{
		if (!(evt instanceof CCndExtension) && !(evt instanceof CActExtension))
			android.util.Log.e("fontembed", "darkedif_jni_getActionOrConditionIntParam: The CEvent is not CActExtension or CCndExtension. Type is \"" + evt.getClass().getSimpleName() + "\".");
		if (paramIndex == -1)
			android.util.Log.e("fontembed", "darkedif_jni_getActionOrConditionIntParam: paramIndex -1.");
		...
		if (paramType == 29)
		{
			...
			{
				android.util.Log.e("fontembed", "darkedif_jni_getActionOrConditionIntParam: Passed a direction " + result + " which cannot translate into NewDirection bitmask. Using no direction.");
				return 0;
			}
			return 1 << result;
		}
		...
		if (paramType != 22)
			android.util.Log.w("fontembed", "darkedif_jni_getActionOrConditionIntParam: Unknown param type " + paramType + " requested. Defaulting to Expression.");
		return this.rh.get_EventExpressionInt((CParamExpression)evt.evtParams[paramIndex]);
	}
}
