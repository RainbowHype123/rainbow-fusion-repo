...
struct CustomPropertyUpgrader : DarkEdif::Properties::PropertyReader
{
    ...
    void GetProperty(std::size_t jsonID, DarkEdif::Properties::ConverterReturn* const convRet)
    {
        return convRet->Return_Pass();
    }

    void GetPropertyCheckbox(std::size_t jsonID, DarkEdif::Properties::ConverterReturn* const convRet)
    {
        return convRet->Return_Pass();
    }
};

CustomPropertyUpgrader UC;
...
#endif
{
	...
	*/
	...
	LinkAction(0, EmbedFont);
	LinkAction(1, RemoveFont);

	/*
	LinkCondition(0, OnFontEmbedded);
	LinkCondition(1, OnFontRemoved);
	*/

	LinkExpression(0, FontsAdded);
...
Extension::~Extension()
{
	// Prevent a memory leak with the embedded fonts
	
	for (auto& font : embeddedFonts)
    {
		RemoveFontResourceEx(font.c_str(), FR_PRIVATE, nullptr);
    }
