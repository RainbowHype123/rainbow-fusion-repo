...
#import "CExtension.h"
#import "CRunfontembed.hpp"

@implementation CRunfontembed : CRunExtension

-(id)init
{
	if (++initedfontembedCPP == 1)
		fontembed_init();
	return [super init];
}
-(void)dealloc
...
-(int)getNumberOfConditions
{
	...
	return fontembed_getNumberOfConditions();
}
-(BOOL)createRunObject:(CFile*)file withCOB:(CCreateObjectInfo *)cob andVersion:(int)version
{
	if (initedfontembedCPP == 0)
		NSLog(@"Init for Font Embed Object was never run!");
	NSLog(@"CreateRunObject for fontembed");
	...
	cptr = fontembed_createRunObject(edPtr, cob, version, self);
	if (!cptr)
		NSLog(@"Failed to createRunObject for Font Embed Object!");
	free(edPtr);
	return cptr != NULL;
}
-(int)handleRunObject
{
	return fontembed_handleRunObject(cptr);
}
-(void)destroyRunObject:(BOOL)bFast
{
	fontembed_destroyRunObject(cptr, bFast);
	cptr = NULL;
	[expRet dealloc];
}
-(BOOL)condition:(int)paramNum withCndExtension:(CCndExtension*)cnd
{
	...
	// by checking num and the return of fontembed_conditionJump, which is either a 32-bit integer,
	...
	return fontembed_conditionJump(cptr, paramNum, cnd) != 0;
}
-(void)action:(int)paramNum withActExtension:(CActExtension*)act
{
	fontembed_actionJump(cptr, paramNum, act);
}
-(CValue*)expression:(int)paramNum
{
	fontembed_expressionJump(cptr, paramNum);
	return expRet;
}
...
extern "C" {
	void DarkEdif_fontembed_generateEvent(void * ext, int code, int param)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		[extRun->ho generateEvent:code withParam:param];
	}
	void DarkEdif_fontembed_reHandle(void * ext)
	{
		CRunExtension * extRun = (CRunfontembed *)ext;
		[extRun->ho reHandle];
	}

	int DarkEdif_fontembed_actGetParamExpression(void * ext, void * act, int paramNum, int paramType)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		...
		if (paramType == 29)
		{
			...
			{
				printf("fontembed: DarkEdif_actGetParamExpression passed a direction %d which cannot translate into NewDirection bitmask. Using no direction.", result);
				return 0;
			}
			return 1 << result;
		}
		...
		if (paramType != 22)
			printf("fontembed: DarkEdif_actGetParamExpression: Unknown param type %d requested. Defaulting to Expression.", paramType);
		return [actExt getParamExpression:extRun->rh withNum:paramNum];
	}
	const char * DarkEdif_fontembed_actGetParamExpString(void * ext, void * act, int paramNum)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		CActExtension * actExt = (CActExtension *)act;
	...
	double DarkEdif_fontembed_actGetParamExpDouble(void * ext, void * act, int paramNum)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		CActExtension * actExt = (CActExtension *)act;
	...
	int DarkEdif_fontembed_cndGetParamExpression(void * ext, void * cnd, int paramNum, int paramType)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		...
		if (paramType == 29)
		{
			...
			if (result < 0 || result > 31)
			{
				printf("fontembed: DarkEdif_cndGetParamExpression passed a direction %d which cannot translate into NewDirection bitmask. Using no direction.", result);
				return 0;
			}
			return 1 << result;
		}
		...
		// ParamExpression is 22; at the moment, no other types are expected
		if (paramType != 22)
			printf("fontembed: DarkEdif_cndGetParamExpression: Unknown param type %d requested. Defaulting to Expression.", paramType);
		return [cndExt getParamExpression:extRun->rh withNum:paramNum];
	}
	const char * DarkEdif_fontembed_cndGetParamExpString(void * ext, void * cnd, int paramNum)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		CCndExtension * cndExt = (CCndExtension *)cnd;
	...
	double DarkEdif_fontembed_cndGetParamExpDouble(void * ext, void * cnd, int paramNum)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		CCndExtension * cndExt = (CCndExtension *)cnd;
	...
	int DarkEdif_fontembed_expGetParamInt(void * ext)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		return [[extRun->ho getExpParam] getInt];
	}
	const char * DarkEdif_fontembed_expGetParamString(void * ext)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		return [[[extRun->ho getExpParam] getString] UTF8String];
	}
	float DarkEdif_fontembed_expGetParamFloat(void * ext)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		return (float)[[extRun->ho getExpParam] getDouble];
	}

	void DarkEdif_fontembed_expSetReturnInt(void * ext, int i)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		[extRun->expRet forceInt:i];
	}
	void DarkEdif_fontembed_expSetReturnString(void * ext, const char * str)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		[extRun->expRet forceString:[NSString stringWithUTF8String:str]];
	}
	void DarkEdif_fontembed_expSetReturnFloat(void * ext, float f)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
		[extRun->expRet forceDouble:(double)f];
	}

	void DarkEdif_fontembed_freeString(void * ext, const char * cstr)
	{
		[(NSString*)cstr release];
	}

	int DarkEdif_fontembed_getCurrentFusionEventNum(void * ext)
	{
		CRunfontembed * extRun = (CRunfontembed *)ext;
	...
	char * DarkEdif_fontembed_makePathUnembeddedIfNeeded(void * ext, const char * fileName)
	{
	...
	void * DarkEdif_fontembed_getCPtr(void * ext)
	{
		CRunfontembed * const extRun = (CRunfontembed *)((CExtension *)ext)->ext;

		// While we cast this to CRunfontembed, it may be any sort of CRun, including one that doesn't have cptr.
		...
		{
			printf("fontembed: DarkEdif_getCPtr passed an invalid, non-DarkEdif ext.\n");
			return NULL;
		}
		return extRun->cptr;
	}
