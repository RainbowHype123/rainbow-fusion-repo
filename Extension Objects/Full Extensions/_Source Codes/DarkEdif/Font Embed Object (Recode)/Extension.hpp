...
public:
	...
	*/
	...
	//int embeddedFonts;
	std::vector<std::wstring> embeddedFonts;

	// Test parameter for switching between runtimes. Feel free to remove this if it doesn't look useful.
	/*
	#ifdef _WIN32
		int runtimeParam = 1;
	#elif defined(__APPLE__)
	...
	*/
	...
	void EmbedFont(const TCHAR * fontPath);
	void RemoveFont(const TCHAR * fontPath);

	// ~~~ Conditions ~~~
	/*
	bool OnFontEmbedded(const TCHAR * fontPath);
	bool OnFontRemoved(const TCHAR * fontPath);
	*/
