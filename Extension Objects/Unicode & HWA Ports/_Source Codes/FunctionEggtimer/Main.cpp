...
{
	TCHAR * p1=(LPTSTR)param1;
	//LPCTSTR p1 = (LPCTSTR)param1;

	if( _tcscmp( rdPtr->LastFuncName , p1 ) == 0 )
		return true;
...
short WINAPI DLLExport acDelayFunction(LPRDATA rdPtr, long param1, long param2)
{
	...
	FuncStruct NewTimer;
	int strl = _tcslen(p1)+1;
	NewTimer.FuncName = new TCHAR[strl];

	if(NewTimer.FuncName)
		_tcscpy_s(NewTimer.FuncName, strl, p1);
	else return 0;
...
short WINAPI DLLExport actCancelFunction(LPRDATA rdPtr, long param1, long param2)
{
	...
	{
		if( _tcscmp( p1, rdPtr->FunctionList->GetItem(i)->FuncName) == 0 )
		{
...
short WINAPI DLLExport acImmediateFunction(LPRDATA rdPtr, long param1, long param2)
{
	...
	rdPtr->LastFuncName = new TCHAR[strl];
	_tcscpy_s(rdPtr->LastFuncName, strl, p1);
...
long WINAPI DLLExport extLastFunctionName(LPRDATA rdPtr, long param1)
{
	...
	LPTSTR pText = (LPTSTR)callRunTimeFunction(rdPtr, RFUNCTION_GETSTRINGSPACE_EX, 0, (_tcslen(rdPtr->LastFuncName) + 1) * sizeof(TCHAR));
	//_tcscpy(pText, rdPtr->LastFuncName);
	_tcscpy_s(pText, _tcslen(rdPtr->LastFuncName) + 1, rdPtr->LastFuncName);
	return (long)pText;
