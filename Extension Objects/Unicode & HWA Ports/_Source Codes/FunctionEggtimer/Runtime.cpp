...
{
	rdPtr->FunctionList = new AList<FuncStruct>();
	rdPtr->LastFuncName = new TCHAR[1]{ 0 };
...
short WINAPI DLLExport DestroyRunObject(LPRDATA rdPtr, long fast)
{
	...
	*/
	...
	delete rdPtr->FunctionList;
	rdPtr->FunctionList = nullptr;

	delete[] rdPtr->LastFuncName;
	rdPtr->LastFuncName = nullptr;
	...
	for( int i=0; i<rdPtr->FunctionList->Size();i++ )
	{
		...
		if( CurrentStruct->Time <= 0)
		{
			...
			// Update the 'Last function name' in the rdPtr structure
				delete [] rdPtr->LastFuncName;
				int strl = _tcslen(CurrentStruct->FuncName)+1;
				rdPtr->LastFuncName = new TCHAR[strl];
				_tcscpy_s( rdPtr->LastFuncName, strl, CurrentStruct->FuncName);
