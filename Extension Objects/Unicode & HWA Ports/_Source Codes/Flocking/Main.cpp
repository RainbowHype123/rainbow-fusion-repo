...
long WINAPI DLLExport con_onLoopedBoid(LPRDATA rdPtr, long param1, long param2)
{
	...
	//TCHAR* loopName = (TCHAR*)param1;
	TCHAR* loopName = (TCHAR*)CNC_GetStringParameter(rdPtr);
	...
	bool nameEquals = (_tcscmp(loopName, currentLoop.name) == 0);
	if(!nameEquals)
	{
		return false;
	}
...
short WINAPI DLLExport act_loopBoidsInRadius(LPRDATA rdPtr, long param1, long param2)
{
	...
	rdPtr->numLoops++;
	_tcscpy_s(currentLoop.name, /*31*/32, loopname);
	float cellsize = (float)rdPtr->cellsize;
