...
//		{
//			// Gets the string
//			LPTSTR pStr = ((CPropStringValue*)pValue)->GetString();
//
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		is.LoadImage(hInstLib, EXO_ICON, LI_REMAP);	// Load our bitmap from the resource,
														// and remap palette if necessary
		is.SetTransparentColor(is.GetPixelFast(0,0));
...
{
#ifndef RUN_ONLY
//	return &ThisExt.getConditionInfos(code);
...
{
#ifndef RUN_ONLY
//	return &ThisExt.getActionInfos(code);
...
{
#ifndef RUN_ONLY
//	return &ThisExt.getExpressionInfos(code);
