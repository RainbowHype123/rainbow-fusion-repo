...
//	EVPNEXT(rdPtr->rHo.hoCurrentParam);
	switch (CurrentParam->evpCode)
	{
		...
		case PARAM_CMPSTRING: // Trying to get the code to recognize string comparisons.
		{
...
inline bool Param_Comparison_Test(Comparison comparison, const TCHAR* left, const TCHAR* right)
{
	int result = _tcscmp(left, right);
	switch(comparison)
	{
	case PC_EQUAL:
		return result == 0;
