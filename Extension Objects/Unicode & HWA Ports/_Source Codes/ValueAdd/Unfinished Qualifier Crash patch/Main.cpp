...
CONDITION(
	...
	/* Params */			(3,PARAM_OBJECT,_T("Object"),PARAM_STRING,_T("Extended Alt. Value Name"),PARAM_COMPARISON,_T("To Compare Against"))
) {
	...
	TCHAR* p2 = (TCHAR*)Param(TYPE_STRING);
...
CONDITION(
	...
	/* Flags */			EVFLAGS_ALWAYS,
	/* Params */			(3,PARAM_OBJECT,_T("Object"),PARAM_STRING,_T("Extended Alt. Sring Name"),PARAM_CMPSTRING,_T("To Compare Against"))
) {
	// Hack around obscure crash by getting the param data myself instead of trusting Param(TYPE_OBJECT)
	//short p1 = ((eventParam*)Param(TYPE_OBJECT))->evp.evpW.evpW0;
	short p1 = rdPtr->rHo.hoCurrentParam->evp.evpW.evpW0; Param(TYPE_OBJECT);
	...
	//eventParam* comparisonParam = rdPtr->rHo.hoCurrentParam;
	//Comparison comparison = (Comparison)comparisonParam->evp.evpW.evpW0;

	TCHAR* p4 = (TCHAR*)Param(TYPE_STRING);
	...
	if ( p1 & 0x8000 ) // dealing with a qualifier...
	{
			...
			CurrentQualToOi = PtrAddBytes(CurrentQualToOi, 4) )
		{
			...
			for(int i = 0; i < iCount; i++)
			{
				...
				if ( Param_Comparison_Test( (Comparison)p3->compType, value, p4 ) )
				//if (Param_Comparison_Test(comparison, value, p4))
				{
					if ( numSelected++ == 0 )
					{
						CurrentOi->oilListSelected = CurrentObject->hoNumber; //select the first object
...
ACTION(
	...
	/* Flags */			0,
	/* Params */			(3,PARAM_OBJECT,_T("Object"),PARAM_STRING,_T("Extended Alt. Value's Name"),PARAM_NUMBER,_T("New Value"))
) {
...
ACTION(
	...
	/* Flags */			0,
	/* Params */			(4,PARAM_OBJECT,_T("Object"),PARAM_STRING,_T("Extended Alt. Value's Name"),PARAM_NUMBER,_T("Start Value"),PARAM_NUMBER,_T("Step Value"))
) {
	object* p1 = (object*)Param(TYPE_OBJECT);
	TCHAR* p2 = (TCHAR*)Param(TYPE_STRING);
	float p3 = Long2Float(CNC_GetFloatParameter(rdPtr));
...
ACTION(
	...
	/* Flags */			0,
	/* Params */			(3,PARAM_OBJECT,_T("Object"),PARAM_STRING,_T("Extended Alt. String's Name"),PARAM_STRING,_T("New Value"))
) {
	object* p1 = (object*)Param(TYPE_OBJECT);
	TCHAR* p2 = (TCHAR*)Param(TYPE_STRING);
	TCHAR* p3 = (TCHAR*)Param(TYPE_STRING);
...
EXPRESSION(
	...
	/* Name */			"alt(",
	/* Flags */			0,
	/* Params */			(2,EXPPARAM_NUMBER,_T("Fixed Value of Object"),EXPPARAM_STRING,_T("Extended Alt. Value's Name"))
) {
	unsigned int p1 = ExParam(TYPE_INT);
	TCHAR* p2 = (TCHAR*)ExParam(TYPE_STRING);
...
EXPRESSION(
	...
	/* Params */			(2,EXPPARAM_NUMBER,_T("Fixed Value of Object"),EXPPARAM_STRING,_T("Extended Alt. String's Name"))
) {
	unsigned int p1 = ExParam(TYPE_INT);
	TCHAR* p2 = (TCHAR*)ExParam(TYPE_STRING);
