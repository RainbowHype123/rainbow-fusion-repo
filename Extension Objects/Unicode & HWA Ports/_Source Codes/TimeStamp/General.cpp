...
{
	if(edPtr->eHeader.extVersion>KCX_CURRENT_VERSION) // Newer
	{
		int returnbutton = MessageBox(0, "This was created with a newer version of the TimeStamp Extension and is probably incompatable.\n"
		                                 "Please make sure you have the newest version installed, then reload the file.\n"
		                                 "Press ok to load the file anyway.", "TimeStamp", MB_OKCANCEL);
		switch(returnbutton)
		{
		case IDOK:
			return 0;
...
{
	((LPEDATA)OldEdPtr)->eHeader.extVersion=KCX_CURRENT_VERSION;
