...
HGLOBAL WINAPI DLLExport UpdateEditStructure(mv __far *mV, void __far * OldEdPtr)
{
	...
	HGLOBAL hgNew = NULL; // Previously didn't set it to NULL
	...
	{
		char buffer[128];
		sprintf(buffer, "Unknown editdata version: %lu", ((tagEDATA_V1*)OldEdPtr)->eHeader.extVersion);

		MessageBoxA(NULL, buffer, "Parallaxer", MB_OK);
