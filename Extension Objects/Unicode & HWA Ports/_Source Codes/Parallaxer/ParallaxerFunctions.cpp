...
{
	TCHAR output [1024];
	//TCHAR outputMsgBox[1024];
	_stprintf( output, _T("%s: %i"), string, value );
	MessageBox(NULL, output, _T(""), NULL);
}
...
void Debug( char * string, int valueA, int valueB )
{
	TCHAR output[1024];
	//TCHAR outputMsgBox[1024];
	_stprintf( output, _T("%s: %i   %i"), string, valueA, valueB );
	MessageBox(NULL, output, _T(""), NULL);
}
...
/*
		...
		if (rdPtr->isHWA)
		{
			if ( GetSurfacePrototype( &pProto, ps->GetDepth(), ST_HWA_ROMTEXTURE, ps->GetDriver()) ) // Using HWA surface types
			{
				...
				char buffer[128];
				sprintf_s(buffer,
					"finalImage: Type=%d Driver=%d\n",
					rdPtr->finalImage->GetType(),
					rdPtr->finalImage->GetDriver());
...
// ---------------------------------------
BOOL LoadImageFile(LPRDATA rdPtr, LPSURFACE psf, LPTSTR pFileName, DWORD dwFlags) 
{ 
	BOOL    bOK = FALSE; 
