...
int WINAPI DLLExport MakeIconEx ( mv _far *mV, cSurface* pIconSf, LPTSTR lpName, fpObjInfo oiPtr, LPEDATA edPtr )
{
	...
	::LockImageSurface( mV->mvIdAppli, edPtr->imageFrames[0], imageSurface, LOCKIMAGE_READBLITONLY );

		RECT resized = FitBox(imageSurface.GetWidth(), imageSurface.GetHeight(),32,32);
		imageSurface.Stretch(*pIconSf,resized.left,resized.top,resized.right,resized.bottom,0,0,imageSurface.GetWidth(),imageSurface.GetHeight(),BMODE_TRANSP,BOP_COPY,STRF_RESAMPLE );
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		//cSurface is;
		//is.Create(edPtr->swidth, edPtr->sheight, ps);

		// Use a DIB memory surface instead of inheriting the editor's
		// rendering backend. Temporary HWA surfaces disappear when the
		// Frame Editor is highly zoomed in (DX8/DX9/DX11).
		cSurface* proto;
		GetSurfacePrototype(&proto, 24, ST_MEMORY, SD_DIB);

		cSurface is;
		is.Create(edPtr->swidth, edPtr->sheight, proto);
		...
		::LockImageSurface( mV->mvIdAppli, edPtr->imageFrames[0], imageSurface, LOCKIMAGE_READBLITONLY );
			...
			int width = !edPtr->direction ? imageSurface.GetWidth() : edPtr->swidth;
			int height = edPtr->direction ? imageSurface.GetHeight() : edPtr->sheight;

			imageSurface.Stretch(
				is,
				is.GetWidth()/2 - width/2,
				is.GetHeight()/2 - height/2,
				width,
				height,
				0,0,
				imageSurface.GetWidth(),
				imageSurface.GetHeight(),
				BMODE_OPAQUE, BOP_COPY, 0, imageSurface.HasAlpha() ? STRF_COPYALPHA : 0 );

		::UnlockImageSurface( imageSurface );
		
		is.Blit(*ps, x, y, BMODE_TRANSP, BOP_COPY);
	}
#endif // !defined(RUN_ONLY)
}
...
/*	
	...
	if ( fileName != NULL )
	{
		if ( (ext=(LPTSTR)malloc(_MAX_EXT)) != NULL )
		{
...
switch (nPropID)
{
	...
	case PROPID_IMG:{
		int nSize = (1+1) * sizeof(WORD);	// 1st word = number of images, second = single image id

		CPropDataValue* pv = new CPropDataValue(nSize, NULL);
		if ( pv != NULL && pv->m_pData != NULL ){
			LPWORD pw = (LPWORD)pv->m_pData;
			*pw++ = edPtr->numFrames;				// number of images
			*pw++ = edPtr->imageFrames[0];			// image id
			return pv;
			pv->Delete(); // ??
		}
		break;
	...
	case PROPID_ZLENGTH:
		return new CPropDWordValue(edPtr->zLength);
		break;

	case PROPID_LOWERBOUND:
		return new CPropDWordValue(edPtr->lowerLimit);
		break;

	case PROPID_UPPERBOUND:
		return new CPropDWordValue(edPtr->upperLimit);
		break;

	case PROPID_SMOOTHLINES:
		return new CPropDWordValue(edPtr->smoothAmount);
		break;
}
...
BOOL WINAPI DLLExport GetPropCheck(LPMV mV, LPEDATA edPtr, UINT nPropID)
{
	...
	case PROPID_LOWERBOUND:
		return edPtr->hasLowerLimit;
		break;

	case PROPID_UPPERBOUND:
		return edPtr->hasUpperLimit;
		break;

	case PROPID_STATIONATORY:
		return edPtr->stationatory;
		break;

	case PROPID_SMOOTHLINES:
		return edPtr->smoothLines;
		break;
	}
...
HMENU WINAPI DLLExport GetActionMenu(mv _far *mV, fpObjInfo oiPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		HMENU menu = GetPopupMenu(MN_ACTIONS);
...
void WINAPI InitParameter(mv _far *mV, short code, paramExt* pExt)
{
	...
	// Example
	// -------
	// _tcscpy(&pExt->pextData[0], "Parameter Test");
	// pExt->pextSize = sizeof(paramExt) + _tcslen(pExt->pextData)+1;
#endif // !defined(RUN_ONLY)
}
