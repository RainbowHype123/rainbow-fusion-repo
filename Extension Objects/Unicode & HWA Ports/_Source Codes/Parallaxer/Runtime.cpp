...
short WINAPI DLLExport CreateRunObject(LPRDATA rdPtr, LPEDATA edPtr, fpcob cobPtr)
{
	...
	//Acquire the edittime images into the animation list
	for( int i = 0; i < edPtr->numFrames; i++ )
	{
		...
		::LockImageSurface( rhPtr->rhIdAppli, edPtr->imageFrames[i], imageSurface, LOCKIMAGE_READBLITONLY );
			...
			// debug
			char buffer[128];
			sprintf_s(buffer, "parallaxImage: Type=%d Driver=%d\n", parallaxImage->GetType(), parallaxImage->GetDriver());
	...
	//Running HWA? (Shoutout to Looki for this code, via the Surface object)
	#ifdef HWABETA
...
short WINAPI DLLExport DisplayRunObject(LPRDATA rdPtr)
{
	...
	if(finalAlpha)
	{
		// I have Yves' word that GetAlphaPitch is > 0 -Anders
...
/*
	...
	{
		// Rotate quality 1
		//if (rdPtr->rHo.rcAngle && rdPtr->rs.rsFlags & RSFLAG_ROTATE_ANTIA)
		//	flags |= BLTF_ANTIA;
...
{
	OutputDebugString(_T("Start app\n"));
...
{
	TCHAR tutu[100];
	wsprintf(tutu, _T("Start Frame %d\n"), nFrameIndex);
...
void WINAPI DLLExport GetDebugItem(LPTSTR pBuffer, LPRDATA rdPtr, int id)
{
	...
	{
	case DB_OFFSET:
		wsprintf(pBuffer, _T("Offset: %i"), rdPtr->offset);
		break;
	case DB_LOWERBOUND:
		if(!rdPtr->hasLowerLimit)
			wsprintf(pBuffer, _T("Lower bound: (off)"));
		else
			wsprintf(pBuffer, _T("Lower bound: %i"), rdPtr->lowerLimit);
		break;
	case DB_UPPERBOUND:
		if(!rdPtr->hasUpperLimit)
			wsprintf(pBuffer, _T("Lower bound: (off)"));
		else
			wsprintf(pBuffer, _T("Upper bound: %i"), rdPtr->upperLimit );
		break;
	case DB_NUMOBJATTACHED:
			wsprintf(pBuffer, _T("Attached objects: %i"), rdPtr->aObjects->size() );
		break;
	case DB_CURRENTFRAMENUM:
		wsprintf(pBuffer, _T("Current frame: %i"), AnimFrameWrap(rdPtr) );
		break;
	case DB_FRAMEWIDTH:
		{
			int wrappedFrameNum = AnimFrameWrap( rdPtr );
			ParallaxFrame current = rdPtr->frames->at( wrappedFrameNum );
			wsprintf(pBuffer, _T("Frame width: %i"), current.parallaxImage->GetWidth());
			break;
		}
	case DB_FRAMEHEIGT:
		{
			int wrappedFrameNum = AnimFrameWrap( rdPtr );
			ParallaxFrame current = rdPtr->frames->at( wrappedFrameNum );
			wsprintf(pBuffer, _T("Frame height: %i"), current.parallaxImage->GetHeight());
			break;
		}
	case DB_INTERNALWIDTH:
		wsprintf(pBuffer, _T("Internal width: %i"), rdPtr->oldWidth);
		break;
	case DB_INTERNALHEIGHT:
		wsprintf(pBuffer, _T("Internal height: %i"), rdPtr->oldHeight);
		break;
	}
#endif // !defined(RUN_ONLY)
}
